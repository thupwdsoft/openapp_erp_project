...
def create_order(self, **kw):
    ...
    partner_data = json_data.get("user")
    ...
    partner = request.env["res.partner"].sudo().browse(partner_data.get("id"))
    if not partner.exists():
        return make_json_response({"error": "Thông tin user không tồn tại."}, status=400)
    ...
    for item in cart_items:
        ...
        if not product:
            return make_json_response({"error": f"Sản phẩm {product_tmpl_id} không tồn tại!"}, status=404)
...
def get_orders(kw):
    ...
    states = json_data.get("states")  # ["sale", "done", "cancel"]
    ...
    if not partner.exists() or partner.company_id.id != company.id:
        return {"success": False, "error": "Partner không tồn tại hoặc không thuộc công ty"}

    # Tùy theo user_type, xây domain khác nhau
    if partner.user_type == "staff":
        domain = [
            "&",
                ("company_id", "=", company.id),
                "|",
                    ("partner_id", "=", partner.id),
                    ("partner_id.user_type", "=", "customer"),
        ]
    elif partner.user_type == "manager":
        domain = [
            ("company_id", "=", company.id),
        ]
    else:  # customer
